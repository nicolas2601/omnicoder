---
name: learn
description: "Aprende sobre el proyecto actual y guarda memoria persistente en ~/.qwen/memory/ y ./.qwen/memory/"
---
...
- **Global** (`~/.qwen/memory/`): preferencias del usuario, feedback, errores recurrentes.
- **Proyecto** (`./.qwen/memory/`): stack, convenciones, archivos clave, comandos de build/test/lint, decisiones de arquitectura.

## Workflow

### Paso 1: Analizar proyecto actual

1. Lee (si existen): `README.md`, `package.json`, `pyproject.toml`, `Cargo.toml`, `go.mod`, `tsconfig.json`, `.env.example`, `CLAUDE.md`, `QWEN.md`, `AGENTS.md`.
...
Actualiza `~/.qwen/memory/feedback.md` con una entrada nueva:
...
- Si `./.qwen/memory/project.md` ya existe, lee primero y actualiza solo secciones que cambiaron.
